...
tf.app.flags.DEFINE_integer('truncate_frames', 2*15, 'Number of frames to leave in the saved tfrecords')
...
def read_one_video(video_path, jobid):
    ...
    if not ratio:
        print("video ratio is incorrect!", video_path)
        return 0, False
    ...
    if FLAGS.low_res:
        ...
        if len(image_list)<FLAGS.truncate_frames:
            print('Insufficient video size. ' + str(len(image_list)))
            return 0, False
        start_index = (int)((len(image_list) - FLAGS.truncate_frames)/2)
        image_list = image_list[start_index:FLAGS.truncate_frames + start_index + 1]
